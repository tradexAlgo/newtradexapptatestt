...
const MCX = ({ navigation,route }: any) => {
    const { searchQu } = route.params || {};
  ...
  const handleBuySell = (option: any, action: 'BUY' | 'SELL') => {
    ...
    const maincolor = action === 'BUY' ? color.color_darkblue : color.color_red;
    const price = action === 'BUY' ? option.CE_LTP : option.PE_LTP;

    navigation.navigate('CommodityOrderPlacementScreen', {
      maincolor,
      leadtext: action,
      commodity: option.strikePrice,
      price,
      symbol: option.symbol, // Or however you're identifying the commodity
      commodityName: option?.name || 'Commodity',
    });
  };
  ...
  const fetchMarketData = async (payload: any) => {
    ...
    try {
      ...
      const response = await axios.post(STOCK.GET_QOUTES_OLD, payload);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {/* {console.log("kkdkdLiveData", liveData)} */}
      {commodityData && liveData && <View>
        {/* The ommodity details... */}
        <Text style={[styles.headerText, { color: 'white', fontWeight: '600' }]}>{commodityData?.exSymName}</Text>
        <Text style={[styles.headerText, { fontSize: 12, color: color.color_white, marginTop: 10 }]}>{commodityData?.symTicker} · {commodityData?.exchangeName}</Text>

        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 10 }}>

          {liveData && (
            <Text style={[styles.headerText, { fontSize: 20, color: color.color_white, fontWeight: 'bold' }]}>{liveData[0]?.v?.lp ? liveData[0]?.v?.lp : ''}</Text>
          )}
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                navigation.navigate('CommodityOrderPlacementScreen', {
                  ...
                  leadtext: istext,
                  commodity: liveData[0]?.v?.lp,
                  symbol: liveData?.[0]?.n, // Or however you're identifying the commodity
                  commodityName: selectedCommodity || 'Commodity',
                  short_name: liveData[0]?.v?.short_name,
                });
              }}>
